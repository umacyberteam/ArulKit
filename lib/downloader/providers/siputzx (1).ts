...
/** Instagram provider using SiputZX's sssinstagram endpoint. */
export class SiputzxProvider implements DownloaderProvider {
  ...
  async resolve(
    ...
    const formats = rawItems
      .map((item, index) => {
        ...
        return {
          id: `${isAudio ? "audio" : isVideo ? "video" : "image"}-${
            index + 1
          }`,
  ...
  private collectMediaItems(payload: any): any[] {
    const candidates = [
      // Format utama sssinstagram
      payload?.data?.url,

      // Fallback berbagai format response
      payload?.data?.medias,
      payload?.data?.media,
      payload?.data?.result,
      ...
      // Kalau data langsung berupa object yang punya URL
      payload?.data,
    ];
    ...
    for (const candidate of candidates) {
      ...
      if (
        ...
        this.pickUrl(candidate)
      ) {
        return [candidate];
      }
    }

    if (this.pickUrl(payload)) {
      return [payload];
    }
